// POST /api/admin/agents/[id]/approve — PENDING -> VERIFIED.
...
function req(): NextRequest {
  return new NextRequest('http://localhost/api/admin/agents/agent-1/approve', {
    method: 'POST',
...
it('approves a PENDING agent, audits, and notifies the owner', async () => {
  prismaMock.agentProfile.findUnique.mockResolvedValue({
    id: 'agent-1',
  ...
  expect(prismaMock.agentProfile.update).toHaveBeenCalledWith(
    expect.objectContaining({
      data: expect.objectContaining({
        verificationStatus: 'VERIFIED',
        verificationReviewerId: 'admin-1',
      }),
    }),
  );
  expect(mockLogAdminAction).toHaveBeenCalledWith(
    expect.anything(),
    expect.objectContaining({ action: 'agent.verify.approve', targetId: 'agent-1' }),
  );
  expect(mockCreateNotification).toHaveBeenCalledWith(
    expect.anything(),
    expect.objectContaining({ userId: 'owner-1', type: 'VERIFICATION_APPROVED' }),
  );
});
