// GET /api/agent/missions/[id] — Phase 4 mission detail for the owning agent.
...
function req(): NextRequest {
  return new NextRequest('http://localhost/api/agent/missions/mission-1');
}
...
it('returns the mission detail when owned', async () => {
  ...
  expect(body.mission.statusEvents).toEqual([]);
});
