...
// (5 Phase-5 canonical + 1 post-audit email-job-purge + 1 Corridor Sourcing
// Phase 4 mission-auto-deliver (US9) + 2 Corridor Sourcing Phase 7
// subscription-renewal-reminder / subscription-grace-sweep, F27/US8.)
//
...
describe('vercel.json schema (CRON-07, D-20)', () => {
  ...
  it('declares schedules for the 9 canonical crons (Phase 5 + post-audit + Corridor Sourcing Phase 4 + Phase 7)', () => {
    if (!existsSync(VERCEL_JSON)) return;
    const cfg = JSON.parse(readFileSync(VERCEL_JSON, 'utf8')) as VercelConfig;
    ...
    expect(paths).toEqual([
      ...
      '/api/cron/mission-auto-deliver',
      '/api/cron/order-expiration',
      '/api/cron/outbox-drain',
      '/api/cron/subscription-grace-sweep',
      '/api/cron/subscription-renewal-reminder',
